...
class BackupManager:
    ...
    def create_backup(self, files: List[str], note: str = "", 
                     preserve_structure: bool = True) -> str:
        """
        Buat backup dengan semua file dalam satu folder
        
        Argumen:
            files: Daftar jalur file untuk dibackup
            note: Catatan untuk backup
            preserve_structure: Jika True, pertahankan struktur folder relatif dalam backup
        
        Pengembalian:
            Jalur ke file backup ZIP yang dibuat
        """
        try:
            ...
            # Buat struktur folder sementara untuk backup
            ...
            with zipfile.ZipFile(backup_path, 'w', zipfile.ZIP_DEFLATED) as zf:
                for filepath in files:
                    ...
                    # Tentukan nama file dalam zip
                    if preserve_structure:
                        # Buat path relatif yang aman
                        rel_path = os.path.relpath(filepath, start=os.path.dirname(filepath))
                        if rel_path == '.':
                            rel_path = os.path.basename(filepath)
                        zip_path = os.path.join(backup_folder_name, rel_path)
                    else:
                        # Simpan semua file langsung di folder backup
                        zip_path = os.path.join(backup_folder_name, os.path.basename(filepath))
                    
                    # Pastikan path dalam zip aman (prevent zip slip)
                ...
                # Tambahkan manifest ke root zip
                zf.writestr('manifest.json', json.dumps(manifest, indent=2))
                
                # Tambahkan file info tambahan
                backup_info = {
            ...
            # Buat checksum untuk backup
            self.integrity_manager.create_checksum_file(backup_path)
            
            self.integrity_manager.log_audit_event(
                'backup_created', backup_path,
                f"Backup created with {len(manifest['files'])} files in folder '{backup_folder_name}'"
            )
            
            print(f"Backup berhasil dibuat: {backup_path}")
            print(f"Total file: {len(manifest['files'])}")
            print(f"Folder dalam backup: {backup_folder_name}")
    ...
    def create_permission_backup(
        ...
        """
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        backup_name = f"permission_backup_{timestamp}.zip"
        backup_path = os.path.join(self.backup_dir, backup_name)
        ...
        # Simpan sebagai zip dengan folder terorganisir
        with zipfile.ZipFile(backup_path, 'w', zipfile.ZIP_DEFLATED) as zf:
            backup_folder = f"permission_backup_{timestamp}"
            
            # Buat struktur folder dalam zip
            zf.writestr(f"{backup_folder}/permission_changes.json", 
                       json.dumps(backup_data, indent=2))
            
            # Tambahkan manifest
            manifest = {
                "type": "permission_backup",
    ...
    def restore_backup(self, backup_path: str, restore_dir: str) -> Dict:
        """
        Pulihkan cadangan dengan semua file ke dalam satu folder.
        
        Argumen:
            backup_path: Jalur ke file backup ZIP
            restore_dir: Direktori tujuan untuk restore
        ...
        try:
            ...
            results = {'success': True, 'restored': [], 'errors': [], 'backup_folder': None}
            ...
            with zipfile.ZipFile(backup_path, 'r') as zf:
                ...
                # Dapatkan nama folder backup
                backup_folder = manifest.get('backup_folder', '')
                if not backup_folder:
                    # Coba cari dari file pertama
                    first_file = zf.namelist()[0] if zf.namelist() else ''
                    backup_folder = os.path.dirname(first_file)
                
                results['backup_folder'] = backup_folder
                
                # Ekstrak semua file
                for file_info in manifest.get('files', []):
                    try:
                        zip_path = file_info.get('backup_path', '')
                        if not zip_path:
                            # Fallback: gunakan nama file saja
                            zip_path = os.path.basename(file_info.get('original_path', ''))
                        
                        # Ekstrak file
                        extracted_path = zf.extract(zip_path, restore_dir)
                        
                        # Perbaiki permission jika ada
                        if 'permission' in file_info and file_info['permission'] != 'unknown':
                            try:
                                old_mode = int(file_info['permission'], 8)
                                os.chmod(extracted_path, old_mode)
                            except (ValueError, OSError):
                                pass
                        
                        # Verifikasi integritas
                        if 'hash' in file_info:
                            current_hash = self.integrity_manager.calculate_sha256(extracted_path)
                            if current_hash == file_info['hash']:
                                integrity_ok = True
                            else:
                                integrity_ok = False
                                results['errors'].append(f"Integrity check failed for {os.path.basename(extracted_path)}")
                        else:
                            integrity_ok = True
            ...
            self.integrity_manager.log_audit_event(
                ...
                f"Restored {len(results['restored'])} files to {restore_dir}"
            )
    ...
    def restore_permissions_from_backup(self, backup_path: str) -> Dict:
        ...
        try:
            if backup_path.endswith('.zip'):
                with zipfile.ZipFile(backup_path, 'r') as zf:
                    # Cari file permission_changes.json dalam zip
                    for name in zf.namelist():
                        if name.endswith('permission_changes.json'):
                            backup_data = json.loads(zf.read(name))
                            break
                    else:
                        return {'success': False, 'error': "Permission backup not found in ZIP"}
            else:
            ...
            if f.endswith('.zip'):
                ...
                try:
                    ...
                    backups.append({
                        ...
                        'type': backup_type,
                        'folder': backup_folder if 'backup_folder' in locals() else 'unknown'
                    })
                except:
                    pass
...
class RestoreManager:
            ...
            if backup_path.endswith('.zip'):
                with zipfile.ZipFile(backup_path, 'r') as zf:
                    ...
                    try:
                        ...
                        result['backup_folder'] = manifest.get('backup_folder', '')
                        result['preserve_structure'] = manifest.get('preserve_structure', True)
                    except KeyError:
    ...
    def preview_restore(self, backup_path: str, restore_dir: str = None) -> Dict:
        ...
        preview = {
            ...
            'backup_folder': validation.get('backup_folder', '')
        }
        
        if backup_path.endswith('.zip'):
            for file_info in validation['files']:
                zip_path = file_info.get('backup_path', '')
                filename = os.path.basename(zip_path if zip_path else file_info.get('original_path', ''))
                
                if restore_dir:
                    target_path = os.path.join(restore_dir, filename)
                else:
                    target_path = os.path.join(os.path.dirname(file_info.get('original_path', '')), filename)
                ...
                if file_info.get('permission'):
                    preview['permissions_to_restore'].append({
                        'filepath': target_path,
                        'permission': file_info.get('permission')
                    })
    ...
    def restore_files(
        ...
        result['backup_folder'] = validation.get('backup_folder', '')
            ...
            with zipfile.ZipFile(backup_path, 'r') as zf:
                ...
                for file_info in manifest.get('files', []):
                    ...
                    try:
                        ...
                        zf.extract(zip_path, restore_dir)
                        
                        # Pindahkan ke lokasi yang benar
                        extracted_path = os.path.join(restore_dir, zip_path)
        ...
        try:
            with zipfile.ZipFile(backup_path, 'r') as zf:
                ...
                for f in manifest.get('files', []):
                    if os.path.basename(f.get('original_path', '')) == filename:
                        file_info = f
                        break
                ...
                if target_path:
                    ...
                    result['target_path'] = file_info.get('original_path', filename)
                ...
                with zf.open(zip_path) as source, open(result['target_path'], 'wb') as dest:
                    dest.write(source.read())
                
                # Restore permissions if requested
                if restore_permission and 'permission' in file_info and file_info['permission'] != 'unknown':
                    try:
                        old_mode = int(file_info['permission'], 8)
                        os.chmod(result['target_path'], old_mode)
